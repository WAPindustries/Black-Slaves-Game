const Intro = [
	"In Rakhusa, a village in India, there are 1 million child labourers due to new garment factories built in the village.",
	"As the government of India, you must curb the problem of child labour, through policies and initiatives."
]
...
function Init(){
	...
	Array(canvas, slave_count, info_bg, score_bg, change_div).forEach(elem=>container.appendChild(elem))
	slave_count.style.display = "none"
...
function StartGame(){
	...
	slave_count.style.display = "block"
...
info_btn.onclick = ()=> {
	DisplayInfo(Intro, Game.iTextIndex+=1)
}


const change_div = document.createElement("div")
change_div.style = `position: absolute; margin: auto; user-select: none; 
					border: none; color: white; text-align: center`
change_div.style.backgroundSize = "100% 100%"
change_div.style.width = canvas.width/5
change_div.style.height = canvas.height/10
