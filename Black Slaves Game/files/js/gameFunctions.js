...
const ComputeRate = (val)=>(Math.abs(val-Game.iNiggers)/Game.iChangeRate)
...
function Succeed(val, text){
	...
	Game.sText = text
	ChangeNiggers(val, ComputeRate(val), text)
...
function Fail(val){
	...
	SoyJack()
	ChangeNiggers(val, ComputeRate(val))
...
function ShowScore(count=0){
	...
	setTimeout(()=>ShowScore(count+(Game.iNiggers/Game.iChangeRate)), 1)
}
